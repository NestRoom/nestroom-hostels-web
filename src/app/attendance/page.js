...
export default function AttendancePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 2. Get Hostel Config
      const hostelRes = await fetch(`http://localhost:5001/v1/hostels/${hId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const hostelData = await hostelRes.json();
      const h = hostelData.data.hostel;
      setConfig({
      ...
      const recordsRes = await fetch(`http://localhost:5001/v1/hostels/${hId}/attendance`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const triggerSurpriseCheck = async () => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const res = await fetch(`http://localhost:5001/v1/hostels/${hostelId}/attendance/request?isSurprise=true`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  const saveConfig = async (e) => {
    ...
    try {
      const token = localStorage.getItem("accessToken");
      const res = await fetch(`http://localhost:5001/v1/hostels/${hostelId}/attendance/config`, {
        method: 'POST',
        headers: { 
                      ...
                      <td>
                        <div className={styles.residentInfo}>
                          ...
                          <div>
                            ...
                            <div className={styles.residentPhone}>Room {record.residentId?.roomId || "-"}</div>
                          </div>
                        </div>
                      </td>
                      <td>
                        ...
                        {record.geofenceCheck ? (
                          <div className={record.geofenceCheck.withinGeofence ? styles.geoSuccess : styles.geoFail}>
                            {record.geofenceCheck.withinGeofence ? (
                              ...
                              <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg> {record.geofenceCheck.distanceFromHostel}m Outside</>
                            )}
                          </div>
                        ) : (
        ...
        {activeTab === 'history' && (
           <div className={styles.historyContainer}>
              <div style={{ padding: '4rem', textAlign: 'center', color: '#6B7280' }}>
                 <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" style={{ marginBottom: '1rem' }}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                 <h3>Attendance logs will appear here</h3>
                 <p>Select a date to view historical patterns.</p>
              </div>
           </div>
        )}
      </div>
